...
import { 
  CheckCircle2, Circle, Clock, Play, Square, Trash2, Copy, ChevronDown
} from 'lucide-react';
...
export function TaskItem({ task, onUpdate, onDelete, onDuplicate, teamMembers, clients }: any) {
  ...
  useEffect(() => {
    let timer: any;
    if (isRunning) timer = setInterval(() => setElapsed((p: number) => p + 1), 1000);
      ...
      <div className="flex items-center justify-between p-4 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex items-center gap-3 flex-1 overflow-hidden">
          ...
          <div className="flex flex-col truncate">
            ...
            <div className="flex gap-2 mt-1 text-[8px] font-medium uppercase text-[#00a9e0]">
              <span className="bg-[#00a9e0]/10 px-1.5 py-0.5 rounded-[2px]">{task.starttime || '09:00'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 pt-0 bg-white border-t border-neutral-50 space-y-6 animate-in fade-in duration-300">
          <div className="space-y-5 pt-6">
            ...
            <div className="space-y-2">
              ...
              <textarea rows={5} value={localDesc} onChange={(e) => setLocalDesc(e.target.value)} onBlur={() => onUpdate({ description: localDesc })} className="w-full bg-neutral-50/30 border border-neutral-100 rounded-[4px] p-4 text-[16px] text-neutral-600 outline-none focus:border-[#00a9e0] resize-none" placeholder="Nhập yêu cầu tại đây..." />
            </div>
            <div className="grid grid-cols-2 gap-4 pt-2">
              <div className="space-y-1.5">
                <label className="text-[11px] font-normal text-neutral-400 uppercase tracking-widest">Client / Dự án</label>
                <select value={task.client || ''} onChange={(e) => onUpdate({ client: e.target.value })} className="w-full bg-neutral-50 border-none rounded-[4px] px-2 py-2.5 text-[13px] font-medium text-neutral-600 outline-none">
              ...
              <div className="space-y-1.5">
                 ...
                 <input type="time" value={task.starttime || ''} onChange={(e) => onUpdate({ starttime: e.target.value })} className="w-full bg-neutral-50 rounded-[4px] px-2 py-2 text-[13px] font-medium outline-none" />
              </div>
            </div>
            <div className="flex justify-between items-center text-[11px] font-normal text-neutral-300 uppercase pt-4 border-t border-dashed border-neutral-100">
                <span>Created by: <span className="text-[#00a9e0] font-medium">you</span></span>
                <span>{createdTime}</span>
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={onDuplicate} className="p-3 bg-white border border-neutral-100 text-neutral-300 hover:text-[#00a9e0] rounded-[4px]"><Copy className="w-4 h-4 mx-auto" /></button>
            <button onClick={onDelete} className="p-3 bg-white border border-neutral-100 text-neutral-300 hover:text-rose-400 rounded-[4px]"><Trash2 className="w-4 h-4 mx-auto" /></button>
            <button onClick={() => { onUpdate({ actualtime: elapsed }); setIsExpanded(false); }} className="flex-1 bg-[#00a9e0] text-white py-4 rounded-[4px] text-[13px] font-medium uppercase tracking-[0.2em] shadow-lg shadow-[#00a9e0]/10">Cập nhật</button>
          </div>
        </div>
      )}
    </div>
